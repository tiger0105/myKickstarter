...
contract CampaignHQ {
    ...
    function createCampaign(uint _minCont, uint _quorum, uint _fundingGoal, uint _fundingTime) public {
        // Creates contract, deploys and returns address
        Campaign newCampaign = new Campaign(msg.sender, _minCont, _quorum, _fundingGoal, _fundingTime); 
...
contract Campaign {
    ...
    // this modifier will transition to the next state before executing the rest of the decorated function
        ...
        if(state == States.DONATION) {              // Avoid gas cost of checks if not in Donation phase
            if (block.timestamp > fundingExpiry && address(this).balance < fundingGoal) {
            ...
            if (address(this).balance >= fundingGoal) {
                ...
                emit FundingGoalReached(fundingGoal, address(this).balance); 
                emit stateChange(_state);
            }
        }
        _;
    ...
    constructor(address _creator, uint _minCont, uint _quorum, uint _fundingGoal, uint _fundingTime) {
        manager = _creator;
    ...
    function createRequest(string memory description, address recipient, uint value) 
        public onlyManager() isCurrentStateNot(States.DONATION)
    {
    ...
    //--------Fallback Function-----------
    fallback () payable pretransition() external {
